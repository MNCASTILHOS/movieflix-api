...
app.put("/movies/:id", async (req, res) => {
    ...
    try{
        ...
        if(!movie){
            return res.status(404).send({ messaage: "Filme não encontrado" });
        }
    ...
    //retornar o status correto informando qual o filme foi atualizado
    res.status(200).send();
});
...
app.get("/movies/:genderName", async(req, res) => {
    try {
        ...
        const moviesFilteredByGenderName = await prisma.movie.findMany({
            //incluir os campos em forma de texto na pesquisa
            include: {
                genres: true,
                languages: true,
            },
